...
//this call always returns question_id: 1

export const getOneQuestionDetails = async (QUESTION_ID) => {
  console.log('QUESTION_ID FROM DETAIL SCREEN CALL:', QUESTION_ID);
  try {
    const response = await axios.get(`${baseURL}/questions/${QUESTION_ID}`);
    if (response.status === 200) {
      console.log('QUESTION DETAILS ALWAYS RETURNS ID 1:', response.data);
      return response.data;
    }
  } catch (error) {
    console.log(error);
...
export const shareQuestion = async () => {
  try {
    const response = await axios.post(
      `${baseURL}/share?destination_email=${destination_email}&content_url=${content_url}`
    );
    if (response.status === 200) {
